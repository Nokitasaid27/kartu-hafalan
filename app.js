...
function jamaahList(q){const arr=(db.profiles.pembimbing.jamaah||[]).map((x,i)=>({x,i})).filter(({x})=>x.name.toLowerCase().includes(q.toLowerCase()));if(!arr.length)return `<div class="empty">Belum ada jamaah.</div>`;return `<div class="list">${arr.map(({x,i})=>`<button class="list-item" onclick="openJamaah(${i})"><div class="avatar">${initials(x.name)}</div><div class="grow"><b>${esc(x.name)}</b><div class="small muted">Juz 30</div></div><div>›</div></button>`).join("")}</div>`}
...
function returnToMain(){if(confirm("Kembali ke menu utama? Data hafalan dan profil Anda tetap tersimpan.")){db.mode=null;save();render()}}
